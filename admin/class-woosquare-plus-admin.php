...
class Woosquare_Plus_Admin {
	...
	public function check_for_auth(){
			...
			update_option('woo_square_auth_response',array_map( 'sanitize_text_field', $_REQUEST ));
			update_option('woo_square_access_token',sanitize_text_field($_REQUEST['access_token']));
			update_option('woosquare_plus_reauth_notification',sanitize_text_field($_REQUEST['access_token']));
			update_option('woo_square_refresh_token',sanitize_text_field($_REQUEST['refresh_token']));
	...
	function woosquare_plus_notify(){
		...
		$message = esc_html__( $woosquare_plus_notification->msg, 'sample-text-domain' );
		// printf( '<div class="%1$s"><p>%2$s</p></div>', esc_attr( $class ), esc_html( $message ) );

		echo sprintf('<div class="notice notice-%1$s"><p>%2$s</p></div>', esc_html($ss), esc_html($message));
	...
	public function square_auth_page() {
		...
		if (isset($_SERVER['REQUEST_METHOD']) && 'POST' == sanitize_text_field(wp_unslash($_SERVER['REQUEST_METHOD']))) {
			...
			if (isset($_POST['woo_square_access_token'])) {
				...
				$woo_square_access_token = sanitize_text_field(wp_unslash($_POST['woo_square_access_token']));
			...
			if (isset($_POST['woo_square_settings'])) {
				...
				if( !empty($_POST['woo_square_location_id'])){
					...
					$location_id = sanitize_text_field(wp_unslash($_POST['woo_square_location_id']));
